import {
  DEFAULT_SUPPORT_WHATSAPP,
  formatWhatsAppDisplay,
  whatsAppHref,
} from "@/lib/support-contact";
import { cn } from "@/lib/utils";

type Props = {
  className?: string;
  /** Variante compacte pour l'écran « vérifie ton email ». */
  compact?: boolean;
};

export function SignupWhatsAppHelp({ className, compact }: Props) {
  const display = formatWhatsAppDisplay(DEFAULT_SUPPORT_WHATSAPP);
  const href = whatsAppHref(DEFAULT_SUPPORT_WHATSAPP);

  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      className={cn(
        "block brutal-border-thin border-[#25D366] bg-[#25D366]/10 p-4 hover:bg-[#25D366]/15 transition-colors group",
        className,
      )}
    >
      <div className="flex items-start gap-3">
        <span
          className="flex-shrink-0 w-10 h-10 rounded-full bg-[#25D366] text-white flex items-center justify-center brutal-border-thin border-[#128C7E]"
          aria-hidden
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
            <path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 018.413 3.488 11.82 11.82 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.51 5.26L3.673 19.5l3.981-1.307zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z" />
          </svg>
        </span>
        <div className="min-w-0 flex-1">
          <div className="text-[10px] uppercase tracking-widest font-black text-[#128C7E] mb-1">
            Besoin d'aide ?
          </div>
          {!compact && (
            <p className="text-sm text-muted-foreground leading-snug mb-2">
              Un souci pour créer ton compte ? Écris-nous sur WhatsApp, on te répond rapidement.
            </p>
          )}
          <div className="font-mono text-base sm:text-lg font-black text-foreground group-hover:text-[#128C7E] transition-colors break-all">
            {display}
          </div>
          <div className="text-[10px] uppercase tracking-widest font-bold text-[#25D366] mt-1">
            Ouvrir WhatsApp →
          </div>
        </div>
      </div>
    </a>
  );
}
